<?php
	...
	foreach ($students as $studentName) {
		$sql3 = "INSERT INTO ExamList(Username, ExamName, ExamStatus) VALUES('$studentName', '$examName', '$examStatus')";
	...
	if (isset($_POST['examName'], $_POST['questionList'], $_POST['pointList'])) {
	   $examInsert = "INSERT INTO Exams (ExamName, QuestionList, PointList, ExamStatus)  VALUES ('$examName','$questionList','$pointList', '$examStatus');";
					   
		$assignExam = "UPDATE ExamList SET ExamStatus = '$examStatus' WHERE ExamName = '$examName';";
		if(mysqli_query($db, $examInsert) && mysqli_query($db, $assignExam)) {
			$message = "success";
		} else {
			$message = "fail";
		}
