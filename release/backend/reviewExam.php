<?php 
  ...
  if (isset($_POST['examName'], $_POST['username'])) {
    ...
    for ($i=0; $i <= count($id_array) - 1; $i++) {
      $id = $id_array[$i];
      $pt = $pt_array[$i];
      $id_query = "SELECT * FROM QuestionBank INNER JOIN StudentAnswers ON QuestionBank.ID = StudentAnswers.ID WHERE StudentAnswers.Username = '$username' AND QuestionBank.ID = '$id';";
